...
export async function initDb(): Promise<void> {
  ...
  try {
    ...
    if (backend.kind === 'pglite' && config.dbFile !== ':memory:' && CORRUPTION_RE.test((err as Error).message)) {
      console.warn(`[db] local PGlite failed schema init — recreating ${config.dbFile}. (${(err as Error).message})`);
