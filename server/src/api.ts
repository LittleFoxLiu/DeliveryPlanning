...
api.get('/directory/merchants', authenticate(true), h(async (_req, res) => {
  const merchantRows = await merchants.list();
  res.json({
    merchants: await Promise.all(merchantRows.map(async (m) => ({
      id: m.id,
      name: m.name,
...
api.post('/merchant/orders', ...merchantOnly, h(async (req, res) => {
  ...
  const customer = await customers.create(customerName);
...
api.post('/merchant/orders/:id/ready', ...merchantOnly, h(async (req, res) => {
  ...
  res.json({
    ...
    dispatch: outcome,
    delivery: (await deliveries.byOrderId(order.id)) ? deliveryView((await deliveries.byOrderId(order.id))!) : null,
  });
}));
...
api.get('/customer/orders', ...customerOnly, h(async (req, res) => {
  res.json({ orders: await Promise.all((await orders.byCustomer(req.user!.refId!)).map(orderView)) });
}));
...
api.post('/driver/deliveries/:id/accept', ...driverOnly, h(async (req, res) => {
  const delivery = await ownedDeliveryForDriver(req, idParam(req.params.id, 'delivery id'));
  const updated = await driverProgress(delivery.id, req.user!.refId!, 'accept');
...
api.post('/driver/deliveries/:id/status', ...driverOnly, h(async (req, res) => {
  ...
  const delivery = await ownedDeliveryForDriver(req, idParam(req.params.id, 'delivery id'));
  const updated = await driverProgress(delivery.id, req.user!.refId!, action);
  res.json({ delivery: deliveryView(updated) });
}));
...
api.get('/admin/overview', ...adminOnly, h(async (_req, res) => {
  const [activeOrders, allDrivers, activeDeliveries, trafficRows, roadRows, events] = await Promise.all([orders.active(), drivers.all(), deliveries.active(), traffic.all(), roads.all(), listEvents({ limit: 60 })]);
  res.json({
    orders: await Promise.all(activeOrders.map(async (o) => { const d = await deliveries.byOrderId(o.id); return { ...await orderView(o), delivery: d ? deliveryView(d) : null }; })),
    ...
    assignments: (await Promise.all(activeOrders.map((o) => assignmentReasoningView(o.id)))).flat().filter((a) => a.status === 'active'),
    traffic: trafficRows,
...
api.get('/admin/orders', ...adminOnly, h(async (_req, res) => {
  ...
  res.json({ orders: await Promise.all(list.map(async (o) => { const d = await deliveries.byOrderId(o.id); return { ...await orderView(o), delivery: d ? deliveryView(d) : null }; })) });
}));
